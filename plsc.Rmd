...
A responsive custom theme for making figures
...
Group1.X %>%
  ...
  imap(~ ggplot(Group1.X, aes(x = .data[[.y]])) +
...
```{r fig.width=12, fig.height=3, dpi=150, message=FALSE, warning=FALSE}
fit_resi <- file.path(fig_dir, "resi-fit_rsfc.png")

# --- Data prep (safe names + numeric vectors) ---
resid_vec <- as.numeric(Group1_residuals[, 1])
f1 <- as.numeric(fitted(lm.Group1))
r1 <- as.numeric(residuals(lm.Group1))

resid_df <- data.frame(resid = resid_vec)
fit_df   <- data.frame(fitted = f1, resid = r1)

# --- Plots ---
# (1) Q–Q plot — lavender
p1 <- ggplot(resid_df, aes(sample = .data$resid)) +
  stat_qq(color = "midnightblue") +
  stat_qq_line(color = "midnightblue") +
  labs(
    title = "Q–Q plot for RSFC residuals",
    x = "Theoretical Quantiles", y = "Sample Quantiles"
  ) +
  custom_theme(12)

# (2) Histogram — pink
p2 <- ggplot(resid_df, aes(x = .data$resid)) +
  geom_histogram(bins = 30, fill = "lavender") +
  labs(
    title = "Histogram of RSFC residuals",
    x = "Residuals", y = "Count"
  ) +
  custom_theme(12)

# (3) Fitted vs Residuals — blue
p3 <- ggplot(fit_df, aes(x = .data$fitted, y = .data$resid)) +
  geom_point(alpha = 0.6, color = "pink") +
  geom_hline(yintercept = 0, linetype = "dashed") +
  labs(
    title = "Fitted vs. RSFC residuals",
    x = "Fitted values", y = "Residuals"
  ) +
  custom_theme(12)

# Combine (1 row x 3 columns) and show inline
combined <- p1 | p2 | p3

# Save the same combined plot as PNG at 300 dpi
ggsave(
  filename = fit_resi,
  plot = combined,
...
```{r fig.width=12, fig.height=4, dpi=150, message=FALSE, warning=FALSE}

# =========================
# 1) Combined 1×3 (Group2)
# =========================
fit_resi_soc <- file.path(fig_dir, "resi-fit_socio.png")

resid_vec2 <- as.numeric(Group2_residuals[, 1])
f2 <- as.numeric(fitted(lm.Group2))
r2 <- as.numeric(residuals(lm.Group2))

resid_df2 <- data.frame(resid = resid_vec2)
fit_df2   <- data.frame(fitted = f2, resid = r2)

# (1) Q–Q — midnightblue
p1 <- ggplot(resid_df2, aes(sample = .data$resid)) +
  stat_qq(color = "midnightblue") +
  stat_qq_line(color = "midnightblue") +
  labs(
    title = "Q–Q plot for Sociocultural residuals",
    x = "Theoretical Quantiles", y = "Sample Quantiles"
  ) +
  custom_theme(12)

# (2) Histogram — lavender
p2 <- ggplot(resid_df2, aes(x = .data$resid)) +
  geom_histogram(bins = 30, fill = "lavender") +
  labs(
    title = "Histogram of Sociocultural residuals",
    x = "Residuals", y = "Count"
  ) +
  custom_theme(12)

# (3) Fitted vs Residuals — pink
p3 <- ggplot(fit_df2, aes(x = .data$fitted, y = .data$resid)) +
  geom_point(alpha = 0.6, color = "pink") +
  geom_hline(yintercept = 0, linetype = "dashed") +
  labs(
    title = "Fitted vs. Sociocultural residuals",
    x = "Fitted values", y = "Residuals"
  ) +
  custom_theme(12)

combined_soc <- p1 | p2 | p3


# Save 1×3 panel
ggsave(
  filename = fit_resi_soc,
  plot = combined_soc,
  width = 12, height = 4, units = "in", dpi = 300
)

# ==========================================
# 2) Q–Q grid for ALL Group2 residual cols
# ==========================================
qq_png <- file.path(fig_dir, "qq-plots_residuals.png")

resid_mat <- as.data.frame(Group2_residuals)
if (is.null(colnames(resid_mat))) {
  colnames(resid_mat) <- paste0("col", seq_len(ncol(resid_mat)))
}

qq_list <- lapply(colnames(resid_mat), function(nm) {
  ggplot(data.frame(resid = as.numeric(resid_mat[[nm]])), aes(sample = .data$resid)) +
    stat_qq(color = "midnightblue") +
    stat_qq_line(color = "midnightblue") +
    labs(title = paste0("Q–Q: ", nm), x = "Theoretical", y = "Sample") +
    custom_theme(4)
})

ncol_grid <- 5
nrow_grid <- ceiling(length(qq_list) / ncol_grid)

qq_grid <- wrap_plots(qq_list, ncol = ncol_grid)

# Save Q–Q grid
ggsave(
  filename = qq_png,
  plot = qq_grid,
  width = 12, height = 2 * nrow_grid, units = "in", dpi = 300
)

# ==================================================
# 3) Scatter (residuals vs. index) for ALL columns
# ==================================================
scatter_png <- file.path(fig_dir, "scatter-plots_residuals.png")

scatter_list <- lapply(colnames(resid_mat), function(nm) {
  df <- data.frame(
    idx   = seq_along(resid_mat[[nm]]),
    resid = as.numeric(resid_mat[[nm]])
  )
  ggplot(df, aes(x = .data$idx, y = .data$resid)) +
    geom_point(alpha = 0.6, color = "pink") +
    geom_hline(yintercept = 0, linetype = "dashed") +
    labs(title = paste0("Residuals: ", nm), x = "Index", y = "Residuals") +
    custom_theme(5)
})

scatter_grid <- wrap_plots(scatter_list, ncol = ncol_grid)

# Save scatter grid
ggsave(
  filename = scatter_png,
  plot = scatter_grid,
  width = 12, height = 2 * nrow_grid, units = "in", dpi = 300
)
